...
import type {
  ...
  RequestWithBody,
  RequestWithQuery,
} from '../models/generic-request.ts'
import { GetAllPublicacionesHandler } from '../handlers/gestion-publicaciones-handlers/get-all/get-all-handler.ts'
import {
  CreatePublicacionesHandler,
  type FormAltaModel,
} from '../handlers/gestion-publicaciones-handlers/create/create-handler.ts'
import {
  DeletePublicacionesHandler,
  type DeleteCommand,
} from '../handlers/gestion-publicaciones-handlers/delete/delete-handler.ts'
import { UpdatePublicacionesHandler } from '../handlers/gestion-publicaciones-handlers/update/update-handler.ts'
import {
  GetPublicacionesHandler,
  type GetPublicacionesCommand,
} from '../handlers/gestion-publicaciones-handlers/get/get-pulicacion-handler.ts'
import {
  ActivarPublicacionesHandler,
  type ActivarCommand,
} from '../handlers/gestion-publicaciones-handlers/activar/activar-handler.ts'
...
router.post('/create', async (req: RequestWithBody<FormAltaModel>, res: any) => {
  ...
  res.status(200).send('Cargado con exito')
})
...
router.post('/delete', async (req: RequestWithBody<DeleteCommand>, res: any) => {
  ...
  res.status(200).send('Suspendida con exito')
})
...
router.post('/activar', async (req: RequestWithBody<ActivarCommand>, res: any) => {
  ...
  res.status(200).send('Activada con exito')
})
